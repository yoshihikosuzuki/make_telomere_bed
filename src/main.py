...
def split_fasta(
    ...
    split_contigs = [
        bs.FastaRecord(
            name=f"{contig.name}/{i*size}_{min((i+1)*size,contig.length)}", seq=seq
        )
...
def run_trf(fasta_fname: str, trf_path: str = "trf", ignore_exist: bool = False) -> str:
    ...
    if not ignore_exist and isfile(out_fname):
        ...
        return out_fname
    command = f"{trf_path} {fasta_fname} 2 7 7 80 10 50 500 -h -ngs > {out_fname}"
            ...
            with open(trf_fname, "r") as f:
                for line in f:
                    ...
                    if line.startswith("@"):
                        ...
                        if (
                            len(useq) == unit_len
                            and r.align(unit_seq, useq).diff == 0.0
                        ):
...
def main():
    ...
    if args.split_contigs:
        args.contig_fasta = split_fasta(
            args.contig_fasta, ignore_exist=args.ignore_exist
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("contig_fasta", type=str, help="A fasta file name.")
    parser.add_argument(
        "unit_sequence",
        type=str,
    ...
    assert 2 <= len(args.unit_sequence) <= 50, "Unit sequence must be from 2-50 bp"
    return args
